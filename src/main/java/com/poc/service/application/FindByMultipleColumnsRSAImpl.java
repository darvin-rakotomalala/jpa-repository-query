...
public class FindByMultipleColumnsRSAImpl implements FindByMultipleColumnsRSA {
    ...
    @Override
    public List<TutorialDTO> getByTitleContainingIgnoreCaseAndPublished(String title, boolean isPublished) {
        return tutorialMapper.toDTO(findByMultipleColumnsRSM.getByTitleContainingIgnoreCaseAndPublished(title, isPublished));
    }
